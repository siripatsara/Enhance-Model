...
class SimpleDataAugmentation:
    ...
    def rotate_image_and_labels(self, image, labels, angle):
        ...
        for label in labels:
            ...
            new_x = cos_a * (abs_x - center[0]) - sin_a * (abs_y - center[1]) + center[0]
            new_y = sin_a * (abs_x - center[0]) + cos_a * (abs_y - center[1]) + center[1]
            
            # Convert back to relative coordinates
            new_x_rel = max(0, min(1, new_x / w))
            new_y_rel = max(0, min(1, new_y / h))
            
            rotated_labels.append([class_id, new_x_rel, new_y_rel, width, height])
    ...
    def add_gaussian_noise(self, image, variance=25):
        ...
        noise = np.random.normal(0, variance**0.5, image.shape).astype(np.uint8)
    ...
    def random_crop_and_resize(self, image, labels, crop_factor=0.8):
        ...
        for label in labels:
            ...
            if (abs_x - abs_w/2 > x_start and abs_x + abs_w/2 < x_start + new_w and
                ...
                adjusted_labels.append([class_id, new_x_rel, new_y_rel, new_w_rel, new_h_rel])
    ...
    def augment_single_image(self, image_path, multiplier=3):
        ...
        # สร้างรูปเพิ่ม
        augmentations = [
            ("rotate", lambda img, lbl: self.rotate_image_and_labels(img, lbl, random.uniform(-15, 15))),
            ("bright", lambda img, lbl: (self.adjust_brightness_contrast(img, random.randint(-30, 30), random.uniform(0.8, 1.2)), lbl)),
            ("noise", lambda img, lbl: (self.add_gaussian_noise(img, random.randint(10, 30)), lbl)),
            ("flip", lambda img, lbl: self.horizontal_flip(img, lbl)),
            ("hsv", lambda img, lbl: (self.change_hue_saturation(img, random.randint(-10, 10), random.uniform(0.8, 1.2), random.uniform(0.8, 1.2)), lbl)),
            ("blur", lambda img, lbl: (self.motion_blur(img, random.choice([3, 5, 7])), lbl)),
            ("crop", lambda img, lbl: self.random_crop_and_resize(img, lbl, random.uniform(0.7, 0.9))),
        ]
        ...
        for i in range(multiplier):
            try:
                ...
                aug_label_path = self.output_labels_dir / f"{image_path.stem}_aug_{i+1}_{aug_suffix}.txt"
    ...
    def augment_dataset(self, multiplier=3):
        ...
        print(f"📈 Generated approximately {len(image_files) * (multiplier + 1)} total images")
